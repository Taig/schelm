...
abstract class AttributesOps[+F[+_, -_], +Event, -Context] {
  def get: Attributes

  def update(f: Attributes => Attributes): F[Event, Context]

  final def set(attributes: Attributes): F[Event, Context] = update(_ => attributes)
}

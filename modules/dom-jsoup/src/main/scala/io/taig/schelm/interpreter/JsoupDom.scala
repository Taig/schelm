...
import io.taig.schelm.data.JsoupPlatform
import org.jsoup.nodes.{Document => JDocument, Element => JElement, Node => JNode, TextNode => JText}

import scala.jdk.CollectionConverters._

final class JsoupDom[F[_]](val document: JDocument)(implicit F: Sync[F]) extends Dom[F] with JsoupPlatform {
  ...
  override def createElement(name: String): F[Element] = document.createElement(name).pure[F]
  ...
  override def childAt(element: JElement, index: Int): F[Option[Node]] =
    F.delay(element.child(index).some).recover { case _: IndexOutOfBoundsException => None }.widen

  override def children(element: JElement): F[List[Node]] = F.delay(element.childNodes().asScala.toList)

  ...
  def apply[F[_]]: Differ[Html[F], HtmlDiff[F]] = new Differ[Html[F], HtmlDiff[F]] {
    override def diff(current: Html[F], next: Html[F]): Option[HtmlDiff[F]] =
      (current.component, next.component) match {
        case (current: Component.Element[F, Html[F]], next: Component.Element[F, Html[F]]) =>
          element(current, next)
        case (current: Component.Fragment[F, Html[F]], next: Component.Fragment[F, Html[F]]) =>
          fragment(current, next)
        case (current: Component.Text[F], next: Component.Text[F]) => text(current, next)
        case _                                                     => HtmlDiff.Replace(next).some
      }

    def element(current: Component.Element[F, Html[F]], next: Component.Element[F, Html[F]]): Option[HtmlDiff[F]] = {
    ...
    def fragment(
        current: Component.Fragment[F, Html[F]],
        next: Component.Fragment[F, Html[F]]
    ): Option[HtmlDiff[F]] =
      children(current.children, next.children)

    def text(current: Component.Text[F], next: Component.Text[F]): Option[HtmlDiff[F]] =
    ...
    def listeners(current: Listeners, next: Listeners): Option[HtmlDiff[F]] =
      if (current.isEmpty && next.isEmpty) None
      else {
        val currentKeys = current.values.keys.toList
        val nextKeys = next.values.keys.toList
    ...
    def indexedChildren(
        previous: Children.Indexed[Html[F]],
        next: Children.Indexed[Html[F]]
    ): Option[HtmlDiff[F]] = {

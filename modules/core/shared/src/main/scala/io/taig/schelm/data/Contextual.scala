...
sealed abstract class Contextual[-Context, +A] extends Product with Serializable {
  final def map[B](f: A => B): Contextual[Context, B] = this match {
    case widget: Contextual.Patch[Context, A] => Contextual.Patch(widget.f, f(widget.widget))
...
object Contextual {
  final case class Patch[Context, A](f: Context => Context, widget: A) extends Contextual[Context, A]

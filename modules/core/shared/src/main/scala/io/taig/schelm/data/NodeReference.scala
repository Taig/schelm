...
object NodeReference {
  ...
  final case class Stateful[F[_], A](identifier: Identifier, reference: A) extends NodeReference[F, A]
    ...
    override def traverse[G[_]: Applicative, A, B](fa: NodeReference[F, A])(f: A => G[B]): G[NodeReference[F, B]] =
      fa match {
        case reference @ NodeReference.Element(component, _) =>
          component.traverse(f).map(node => reference.copy(node = node))
        case reference @ NodeReference.Fragment(component) =>
          component.traverse(f).map(node => reference.copy(node = node))
        case reference @ NodeReference.Stateful(_, node) =>
         f(node).map(node => reference.copy(reference = node))

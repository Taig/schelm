...
object HtmlHydrater {
  def apply[F[_]](dom: Dom[F], html: F[Html[F]])(
    ...
    def listeners(html: Html[F]): Option[Listeners[F]] = html.unfix match {
      case node: Node.Element[F, Listeners[F], Html[F]] => Some(node.tag.listeners)
      case _: Node.Fragment[_]                          => None
      case node: Node.Text[F, Listeners[F]]             => Some(node.listeners)
    }
    ...
    def element(
        reference: NodeReference.Element[F, Listeners[F], HtmlReference[F]],
        path: Path
    ...
    def text(reference: NodeReference.Text[F, Listeners[F]], path: Path): F[HtmlHydratedReference[F]] =
    ...
    def hydrate(html: HtmlReference[F], path: Path): F[HtmlHydratedReference[F]] = html.reference match {
      case reference: NodeReference.Element[F, Listeners[F], HtmlReference[F]] => element(reference, path)
      case reference: NodeReference.Fragment[HtmlReference[F]]                 => fragment(reference, path)
      case reference: NodeReference.Text[F, Listeners[F]]                      => text(reference, path)
    }

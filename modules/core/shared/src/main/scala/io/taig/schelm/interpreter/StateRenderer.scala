...
object StateRenderer {
  def apply[F[_]: Monad, G[_]: NodeTraverse, A](
    ...
    def stateful[C](state: Stateful[F, C, G[Fix[λ[B => State[F, G[B]]]]]], path: Path): F[Fix[G]] = {
    ...
    def render(state: Fix[λ[B => State[F, G[B]]]]): Kleisli[F, Path, Fix[G]] =
      Kleisli { path =>
        state.unfix match {
          case state: Stateful[F, _, G[Fix[λ[B => State[F, G[B]]]]]] => stateful(state, path)
          case Stateless(value)                                      => value.traverseWithKey((key, node) => render(node).run(path / key)).map(Fix.apply)
        }
      }

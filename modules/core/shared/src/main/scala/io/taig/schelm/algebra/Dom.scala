...
abstract class Dom[F[_]] {
  ...
  def addEventListener(target: Dom.EventTarget, name: String, listener: Dom.Listener): F[Unit]
  ...
  def removeEventListener(target: Dom.EventTarget, name: String, listener: Dom.Listener): F[Unit]

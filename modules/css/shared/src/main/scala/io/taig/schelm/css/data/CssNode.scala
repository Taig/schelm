...
object CssNode {
  final case class Styled[Event, A](element: Component.Element[Event, A], style: Style) extends CssNode[Event, A]
  final case class Unstyled[Event, A](node: Component[Event, A]) extends CssNode[Event, A]
    ...
    new CssNavigator[Event, CssNode[Event, A], A] {
      override def attributes(css: CssNode[Event, A], f: Attributes => Attributes): CssNode[Event, A] =
        css match {
          case css: Styled[Event, A] =>
            css.copy(element = Navigator[Event, Component.Element[Event, A], A].attributes(css.element, f))
          case css: Unstyled[Event, A] =>
            css.copy(node = Navigator[Event, Component[Event, A], A].attributes(css.node, f))
        }

      override def listeners(css: CssNode[Event, A], f: Listeners[Event] => Listeners[Event]): CssNode[Event, A] =
        css match {
          case css: Styled[Event, A] =>
            css.copy(element = Navigator[Event, Component.Element[Event, A], A].listeners(css.element, f))
          case css: Unstyled[Event, A] =>
            css.copy(node = Navigator[Event, Component[Event, A], A].listeners(css.node, f))
        }

      override def children(css: CssNode[Event, A], f: Children[A] => Children[A]): CssNode[Event, A] =
        css match {
          case css: Styled[Event, A] =>
            css.copy(element = Navigator[Event, Component.Element[Event, A], A].children(css.element, f))
          case css: Unstyled[Event, A] =>
            css.copy(node = Navigator[Event, Component[Event, A], A].children(css.node, f))
        }
      ...
      override def style(css: CssNode[Event, A], f: Style => Style): CssNode[Event, A] =
        css match {
          ...
          case css: Unstyled[Event, A] =>
            css.node match {
              case node: Component.Element[Event, A] =>

...
import io.taig.schelm.data.{Attributes, Children, Component, Listeners, Widget}

final case class CssWidget[+F[_], -Context](widget: Widget[Context, CssNode[Component[F, CssWidget[F, Context]]]]) extends AnyVal

object CssWidget {
  def toStylesheetHtml[F[_], Context](widget: CssWidget[F, Context], context: Context): CssHtml[F] = ???
//    widget.widget match {
//      case widget: Widget.Patch[Context, CssNode[Component[F, CssWidget[F, Context]]]] =>
...
//    }
  ...
  implicit def navigator[F[_], Context]: CssNavigator[CssWidget[F, Context], CssWidget[F, Context]] = ???

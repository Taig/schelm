...
trait CssNavigator[Event, A, B] extends Navigator[Event, A, B] {
  def style(value: A, f: Style => Style): A
}

object CssNavigator {
  def apply[Event, A, B](implicit navigator: CssNavigator[Event, A, B]): CssNavigator[Event, A, B] = navigator
